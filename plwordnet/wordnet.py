...
class LexicalUnit:
    __slots__ = (
        "id synset name pos_pl pos language domain description rich_description variant tag_count sentiment".split()
    )
    id: int
    name: str
    variant: int
    ...
    def load(self, file, sentiment_file=None, *, clean=True, full_parse=False):
        ...
        if sentiment_file is not None:
            ...
            for lemma, variant, pos, charged, emotions, valuations, polarity, example1, example2 in rows:
                ...
                examples = []
                if example1 and example1 != "NULL":
                    examples.append(example1)
                if example2 and example2 != "NULL":
                    examples.append(example2)
                if not polarity and not emotions and not valuations and not examples:
                    continue
                self.sentiment_count += 1
                sentiment[(lemma, int(variant))].append(
                    EmotionalAnnotation(polarity=polarity, emotions=emotions, valuations=valuations, examples=examples)
                )
        ...
        for e in root.iter("relationtypes"):
            ...
            self.relation_types[id] = self.relation_by_name[name] = RelationType(
                ...
                shortcut=a["shortcut"],
                display=a["display"],
                autoreverse=a["autoreverse"] == "true",
                inverse=inverse,
            )
    ...
    def hypernyms(self, synset, interlingual=False):
        ...
        for rel in self._get_hypernym_relations(interlingual):
            for (
                ...
                _,
                _,
            ) in self.synset_relations_where(predicate=rel, object=synset):
                res.append(s)
...
def parse_description(text):
    ...
    return error, Description(
        qualifier=res.get("K", None),
        definition=res.get("D", ""),
        examples=examples,
        links=links,
